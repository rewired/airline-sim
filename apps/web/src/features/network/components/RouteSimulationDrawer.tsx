...
export function RouteSimulationDrawer({ candidate, isOpen, onClose }: RouteSimulationDrawerProps) {
    const [frequency, setFrequency] = useState(7);
    const [aircraftId, setAircraftId] = useState<string>(MOCK_AIRCRAFT_TYPES[0].id);
    ...
    const handleCreate = async () => {
        try {
            await createRoute.mutateAsync({
                ...
                weeklyDemand: candidate.weeklyDemand || 500,
                competitionScore: candidate.competitionScore || 0.5,
                strategicRole: "trunk"
            });
            onClose();
        } catch (err) {
    ...
    const ac = MOCK_AIRCRAFT_TYPES.find(a => a.id === aircraftId) || MOCK_AIRCRAFT_TYPES[0];
    ...
    const revenue = loadFactor * capacity * (candidate.distanceKm * 0.15); // mock yield
        ...
        <Drawer open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DrawerContent className="max-h-[85vh]">
                <div className="mx-auto w-full max-w-3xl">
                    <DrawerHeader>
                        <DrawerTitle>Simulate Route: {candidate.origin.iata} ⇌ {candidate.destination.iata}</DrawerTitle>
                        <DrawerDescription>Configure frequency and equipment to evaluate economics.</DrawerDescription>
                    </DrawerHeader>
                    {/* ... middle part stays same ... */}
                    <div className="p-4 pb-0 flex flex-col md:flex-row gap-8">
                        <div className="flex-1 space-y-6">
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <Label>Equipment Assignment (Mock)</Label>
                                    <Select value={aircraftId} onValueChange={setAircraftId}>
                                        ...
                                        <SelectContent>
                                            {MOCK_AIRCRAFT_TYPES.map(a => (
                                                <SelectItem key={a.id} value={a.id}>{a.code} ({a.seats} seats)</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
                    ...
                    <DrawerFooter className="flex-row justify-end space-x-2">
                        ...
                        >
                            {createRoute.isPending ? "Saving..." : "Open Route & Save"}
                        </Button>
                    </DrawerFooter>
                </div>
            </DrawerContent>
