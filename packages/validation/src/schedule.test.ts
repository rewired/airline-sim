...
describe("Schedule Validation", () => {
    it("should return valid with warnings if schedule is empty", () => {
        ...
        expect(result.warnings[0].code).toBe("empty_schedule");
    });

    it("should return valid for a basic schedule", () => {
        const legs: FlightLegPlan[] = [
            { id: "l1", routeId: "r1", dayOfWeek: 1, departureTimeLocal: "10:00", arrivalTimeLocal: "20:00", plannedTailId: "t1", plannedAircraftTypeId: "a320" }
        ];
        const aircraft: Record<string, AircraftType> = {
            "a320": { id: "a320", code: "A320", seats: 180, rangeKm: 6000, minTurnaroundMin: 45, costPerBlockHour: 3500, reliabilityBase: 99 }
        };
        const maintenance: MaintenanceWindow[] = [];
